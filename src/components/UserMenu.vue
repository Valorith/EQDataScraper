          ...
          <div class="dropdown-content">
            ...
            <nav class="menu-items">
              ...
              >
                ...
                {{ isValidatingToken ? 'Validating...' : 'Admin Dashboard' }}
              </button>
...
export default {
  ...
  setup() {
    ...
    const handleAdminDashboard = async () => {
      if (isValidatingToken.value) return
      
      isValidatingToken.value = true
      closeMenu()
      
      try {
        // Check if we're in complete dev mode bypass
        const isDevModeBypass = import.meta.env.VITE_APP_MODE === 'development'
        
        if (isDevModeBypass) {
          // In dev mode bypass, always allow admin access
          console.log('🔧 Dev mode bypass - allowing admin access')
          router.push('/admin')
          return
        }
        
        // Check if we're using dev login tokens (both offline and backend dev login)
        const isOfflineDevToken = userStore.accessToken?.startsWith('offline_access_token_')
        const isDevBypassToken = userStore.accessToken === 'dev_bypass_token'
        const isDevMode = import.meta.env.MODE === 'development'
        const hasDevLoginState = sessionStorage.getItem('dev_login_state')
        
        console.log('🔧 Admin Dashboard: Token validation check', {
          isOfflineDevToken,
          isDevBypassToken,
          isDevMode,
          hasDevLoginState: !!hasDevLoginState,
          userRole: userStore.user?.role,
          isAuthenticated: userStore.isAuthenticated,
          accessToken: userStore.accessToken ? `${userStore.accessToken.substring(0, 20)}...` : 'none'
        })
        
        if (isOfflineDevToken || isDevBypassToken || (isDevMode && hasDevLoginState)) {
          // For dev login tokens, skip backend validation and just check basic state
          console.log('🔧 Dev login detected - skipping backend validation')
          if (userStore.isAuthenticated && userStore.user && userStore.user.role === 'admin') {
            router.push('/admin')
          } else {
            console.log('Dev login state invalid, clearing auth state')
            userStore.clearAuth()
          }
        } else {
    ...
    const getClassColor = (className) => {
      ...
      // Fallback class colors while spell system is disabled
      const classColors = {
        'warrior': '#8e2d2d', 'cleric': '#ccccff', 'paladin': '#ffd700',
        'ranger': '#228b22', 'shadowknight': '#551a8b', 'druid': '#a0522d',
        'monk': '#556b2f', 'bard': '#ff69b4', 'rogue': '#708090',
        'shaman': '#20b2aa', 'necromancer': '#4b0082', 'wizard': '#1e90ff',
        'magician': '#ff8c00', 'enchanter': '#9370db', 'beastlord': '#a52a2a',
        'berserker': '#b22222'
      }
      return classColors[className.toLowerCase()] || '#667eea'
    }
    ...
    const getClassDisplayName = (className) => {
      if (!className) return ''
      // Simple capitalization while spell system is disabled
